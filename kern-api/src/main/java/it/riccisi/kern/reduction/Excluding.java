...
/**
 * Reduction removing events that match a filter from the current
 * representation.
 *
 * <p>The filter changes only what iteration exposes after reduction. It does
 * not narrow the dependency filter that protects the observation's
 * {@link Tail}.</p>
 *
 * @param filter The representation filter.
 */
public record Excluding(@NonNull EventFilter filter) implements EventReduction {
    ...
    public <T> T describe(@NonNull final EventReductionSelection<T> selection) {
        return selection.excluding(this.filter);
    }
}