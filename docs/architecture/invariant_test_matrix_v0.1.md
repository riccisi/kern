...
| --- | --- | --- | --- | --- |
| Committed positions are unique, strictly increasing, and have no ordinary-path gaps | Core unit | Executable | `CommitOutcomeInvariantTest`, `AppendCoordinatorInvariantTest` | [#7](https://github.com/riccisi/kern/issues/7) |
| Events inside an accepted append batch keep the caller-provided order | Core unit | Executable | `AppendCoordinatorInvariantTest` | [#14](https://github.com/riccisi/kern/issues/14) |
| Every subject advances by contiguous revisions starting from the first committed event | Core unit | Executable | `AppendCoordinatorInvariantTest` | [#14](https://github.com/riccisi/kern/issues/14) |
